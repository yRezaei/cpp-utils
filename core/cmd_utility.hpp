#ifndef CMD_UTILITY_HPP
#define CMD_UTILITY_HPP
...
//return MAC;
    ...
    inline std::string execute(const std::string& command)
    {
        ...
        * out to confirm output is as expected */
        std::string output(comout, comout + comlen);
        free(comout);
        pclose(fd);
        return output;
    }
...
// Use color::setControlMode to set rang control mode
...
// Use color::setWinTermMode to explicitly set terminal API for Windows
// Calling color::setWinTermMode have no effect on other OS
...
namespace rang_implementation {
    ...
    using enableStd = typename std::enable_if<
      std::is_same<T, color::style>::value || std::is_same<T, color::fg>::value
        || std::is_same<T, color::bg>::value || std::is_same<T, color::fgB>::value
    ...
    inline void setWinSGR(color::bg col, SGR &state) noexcept
    {
        if (col != color::bg::reset) {
    ...
    inline void setWinSGR(color::fg col, SGR &state) noexcept
    {
        if (col != color::fg::reset) {
    ...
    inline void setWinSGR(color::bgB col, SGR &state) noexcept
    {
        state.bgColor = (BACKGROUND_INTENSITY >> 4)
          | ansi2attr(static_cast<BYTE>(col) - 100);
    }

    inline void setWinSGR(color::fgB col, SGR &state) noexcept
    {
        state.fgColor
          = FOREGROUND_INTENSITY | ansi2attr(static_cast<BYTE>(col) - 90);
    }

    inline void setWinSGR(color::style style, SGR &state) noexcept
    {
        switch (style) {
            case color::style::reset: state = defaultState(); break;
            case color::style::bold: state.bold = FOREGROUND_INTENSITY; break;
            case color::style::underline:
            case color::style::blink:
                state.underline = BACKGROUND_INTENSITY;
                break;
            case color::style::reversed: state.inverse = TRUE; break;
            case color::style::conceal: state.conceal = TRUE; break;
            default: break;
        }
    }
...
inline void setWinTermMode(const color::winTerm value) noexcept
{
