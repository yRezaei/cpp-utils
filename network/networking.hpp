...
namespace net
{
    ...
    /* struct IP {
        IP() : ip_({127, 0, 0, 1}) { }
		IP(const std::string& from_str) {
			auto tokkens = str::split(from_str, ".", false);
			if (tokkens.size() == 4) {
				for(auto i = 0; i < 4; i++)
					ip_[i] = (std::uint16_t)std::stoi(tokkens[i]);
			}
			else {
				ASSERT("ERROR --> 'IP(const std::string&)' The given string is does not represent a valide IP address.");
			}
		}
        IP(const std::array<std::uint8_t, 4>& ip) : ip_(ip) { }
        ~IP() {}
        
        std::string to_string() {
            std::stringstream stream;
            //stream << ip_[0] << "." << ip_[1] << "." << ip_[2] << "." << ip_[3];
            //return stream.str();
            return (std::string(std::to_string(ip_[0])) + "."
                    + std::to_string(ip_[1]) + "."
                    + std::to_string(ip_[2]) + "."
                    + std::to_string(ip_[3]));
        }
    ...
    public:
        Client(const std::string& identity, TransportProtocol protocol, const IP& ip, const Port& port) :
                socket_(nullptr), protocol_(protocol), ip_(ip), port_(port)
        {
            socket_ = Context::instance().create_socket(ZMQ_DEALER);
            ASSERT_IF_NULL(socket_, "'Context::instance().create_socket(ZMQ_SocketType::SOCKET_DEALER)' returned nullptr.\nCheck the IP address.");
    ...
    public:
        Server(TransportProtocol protocol, const Port& port) :
                socket_(nullptr), protocol_(protocol), port_(port)
        {
            socket_ = Context::instance().create_socket(ZMQ_ROUTER);
            ASSERT_IF_TRUE(socket_ == nullptr, "'Context::instance().create_socket(ZMQ_SocketType::SOCKET_ROUTER)' returned nullptr.\nCheck the IP address.");
        ...
        bool receive(std::string& reciever_identity, std::stringstream& incoming_msg)
        {
            ...
            socket_->recv(&identity, ZMQ_RCVMORE | ZMQ_NOBLOCK);
            if (identity.size() > 0) {
